...
class FilterSeries(SeriesDatabase, FilterSeriesBase):
        ...
        for ep, entries in series_entries.iteritems():
            ...
            if 'target' in config or 'qualities' in config:
                ...
                if 'deadline' in config:
                    log.debug('Found deadline in config')
                    if self.process_deadline(task, config, ep, entries):
                        continue
                    reason = 'Deadline expired, choosing best available'
    ...
    def process_deadline(self, task, config, episode, entries):
        """
        Runs the timeframe logic to determine if we should wait for a better quality.
        Saves current best to backlog if timeframe has not expired.
        ...
        # parse options
        log.debug('deadline: %s', config['deadline'])
        deadline = datetime.time(datetime.strptime(config['deadline'], "%H:%M"))
